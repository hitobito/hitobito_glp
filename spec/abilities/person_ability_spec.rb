...
describe PersonAbility do
  ...
  context :administrator do
    ...
    let(:mitglied_zurich) {
      Fabricate(Group::KantonZugeordnete::Mitglied.name.to_sym, group: groups(:zurich_zugeordnete))
    }
    ...
    it "may not destroy person if roles in bern and zurich exist" do
      Fabricate(Group::KantonZugeordnete::Mitglied.name.to_sym, person: mitglied_zurich.person,
